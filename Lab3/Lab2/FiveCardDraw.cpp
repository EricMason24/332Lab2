	...
	while (!validR) {
		...
		if (check == "y") {
			...
			while (!validBet) {
				...
				getline(cin, bet);
				if (bet != "1" && bet != "2") {
					cout << "Please enter either 1 or 2 chips to bet";
				}
				else {
					validBet = true;
...
void FiveCardDraw::fcr(int playerPos) {
	...
	while (!resultV) {
		...
		if (result != "f" && result != "c" && result != "r") {
			...
			if (result == "r") {
				cout << "how many chips would you like to raise (1/2)" << endl;
				string raise;
				getline(cin, raise);
				bool validR = false;
				while (!validR) {
	...
	while (!allSet) {
		for (size_t j = 0; j < players.size(); ++j) {
			...
			if (!currP.hasFolded && !currP.outOfChips) {
				cout << currP.name << " is currently betting" << endl;
				cout << currP.name << " has " << currP.chips << " chips" << endl;
				if (!hasBet) {
					hasBet = check(pos);
				}
				else {
					...
					if (currBet != currP.currBet) {
						...
						cout << currP.name << " already holds the max bet, so move to next player" << endl;
					}
				}
			}
		...
		if (hasBet) {
			...
			if (nonFoldPlayers == 1) {
				...
				cout << p.name << " is the last player left in game, so they have won the round" << endl;
				p.chips += pot;
				++p.wins;
...
void FiveCardDraw::winsLosses() {
	...
	for (i = 0; i < temp.size(); ++i) {
		...
		if (!(*temp[i]).hasFolded) {
			cout << "	Hand: " << (*temp[i]).playerHand << "\n" << endl;
		}
		else {
			cout << (*temp[i]).name << " folded, so they did not get a hand" << endl;
		}
	}
}
...
void FiveCardDraw::checkChips() {
	for (size_t i = 0; i < players.size(); ++i) {
		...
		if (p.chips == 0) {
			bool goodR = false;
			while (!goodR) {
				cout << p.name << " is out of chips!" << endl;
